...
export default class WhiteNoise {
  ...
  async init() {
    await this.audioContext.audioWorklet.addModule(
      'src/utils/classes/worklets/whitenoise-processor.js',
    );

    this.node = new AudioWorkletNode(this.audioContext, 'whitenoise');
  ...
  wireUp(node: AudioNode) {
    if (!node) throw new Error('WhiteNoise class wireUp: node is null');
